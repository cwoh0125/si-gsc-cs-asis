...
import java.net.URLEncoder;
   ...
   implements Servlet
 {
   ...
   public void service(HttpServletRequest request, HttpServletResponse res) throws ServletException, IOException
   {
     ...
     String name = request.getParameter("file")== null ? "" : request.getParameter("file");
     String realfile = request.getParameter("realfile") == null ? "" : request.getParameter("realfile");
     ...
     if (realpath.indexOf("/APPDATA/SMILE2/FILE_BOX") == -1) {
       ...
       try
       {
         ...
         if (in != null) try { in.close(); } catch (IOException localException1) {
       ...
       {
         if (in != null) try { in.close(); } catch (IOException localException3) {
           } if (out != null) try { out.close();
           }
