			...
			if((sqlid = dto.getVariableValue(vid.toString())) != null) { 
				...
				if(list.size()>0) {
					try {
						/**
						 * 로그인 실패 시 정보누출
						 * 24년도 1분기 모의해킹 처리 | np847 | 2024.08.01
						 */
						List<Map<String, String>> listMap = null;
						String value = "";
						
						if("COM010.Get_UsrInfo_Chk_SELECT".equals(sqlid.trim()) ||sqlid.trim() == "COM010.Get_UsrInfo_Chk_SELECT") {
							listMap = list;
							value = listMap.get(i).get("PW_YN");
							
							if("N".equals(value) || value == "N") {
								throw new Exception("잘못된 패스워드로 인한 로그인 실패!!");
							} 
						}
						
						dto.getOutdslist().add(dto.setDataSet(list, "SELECT_" + String.valueOf(i) + "_ODATA"));
					} catch (NullPointerException e) {
						extlogger.debug("NullPointerException ::" + e.getMessage());
					} catch (Exception e) {
						extlogger.debug("NullPointerException ::" + e.getMessage());
					}
